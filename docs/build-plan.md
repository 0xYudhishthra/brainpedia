...
| **Day 3** | ENS integration: register `brainpedia.eth` (Sepolia first), wrap, deploy registrars, write text records helper. End-of-day target: `yudhi.brainpedia.eth` resolves with all Brain text records. |
...
| # | Risk | Mitigation |
|---|---|---|
| 1 | ENS Sepolia subname registrar quirks | Validate Day 3 morning. Fallback: deploy `brainpedia.eth` on mainnet. |
