...
**Screen:** Run the mixture query. Natural prompt to type: *"How does a flash loan turn a spot price oracle into a drain, and what actually defends against it?"* — and tell the agent to use the security shortcut, e.g. *"...query the brainpedia network's research topic for this."* (this makes the MCP call pass `topic: "research"`; deterministic routing, no LLM-router call, identical every take). Show the response: the security Brain responding, `verified: true`, and citations `price-oracle-manipulation`, `oracle-manipulation-via-flash-loan` — the same security domain as the vault just minted. Show the payment plan: `yudhi.bpedia.eth` → `0x8C2BE2D7…:3`, `0.001 OG`, mainnet distributor `0x7AF89556…`.
**Say:**
